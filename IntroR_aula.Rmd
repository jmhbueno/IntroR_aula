...
mdb_lucro <- imdb %>% group_by(diretor) %>% 
...
imdb %>% count(diretor) %>% filter(n>=3) %>% select(diretor) %>% arrange(n>3)
...
# obter dois subsets do imdb com colunas distintas (id em comum)

imdb1 <- imdb %>% slice_sample(prop = .5) %>% select(1:8)
imdb2 <- imdb %>% slice_sample(prop = .3) %>% select(1,9:16)

```


```{r}
left_join(imdb1,imdb2)

semi_join(imdb1,imdb2)

anti_join(imdb1,imdb2)

inner_join(imdb1,imdb2)

intersect(imdb1,imdb2)

drop_na(imdb1)
drop_na(imdb2)

inner_join(drop_na(imdb1),drop_na(imdb2))
```

# Gráficos

Gramática dos gráficos: comandos por camadas

Três camadas: dados, estética, geom

```{r}
library(readr)

big_five <- read_rds("big_five.rds")
...
barplot(table(big_five$idade))

big_five %>% filter(idade <= 80) %>% 
ggplot(aes(x = idade, y = ..count..)) +
  geom_bar()
```
...
big_five %>% filter(gênero %in% c("Masculino","Feminino")) %>% 
ggplot(aes(y = extr, x = gênero)) +
  geom_boxplot()
```
...
```{r}
ggplot(big_five20) + 
  geom_point(aes(x = extr, y = amab, shape = gênero), color = "#1D84B5")

ggplot(data = big_five20) +
  geom_point(aes(x = extr, y = amab), 
             fill = "yellow", color = "red", shape = 21, size = 5)
